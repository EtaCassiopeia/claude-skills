...
A repository following this practice has one normative home for design: **`docs/`**. Everything else that talks about
...
This is the project-agnostic copy shipped with the `design-sync` skill; the reference instance is
`rift-cluster:docs/process/design-code-sync.md`. File names are conventions, not requirements.
